...
Пакет `luci-app-ikev2-site-link` - приложение LuCI, которое отправляет
выбранные сервисы и адреса с одного роутера OpenWrt через другой по IKEv2.
Исходный сценарий: трафик YouTube с офисного роутера идёт через домашний, а
домашний продолжает применять к нему свою обычную стратегию обхода DPI.

![Страница IKEv2 Site Link на выходном роутере](docs/images/overview.png)

## Возможности

- одна установка, две роли - `source` и `exit`, роль выбирается в настройках;
- закрытие по отказу: пока туннель недоступен, выбранный трафик не уходит в
  обычный WAN;
- выбор сервисов и адресов на странице **Policy Routing**, атомарное
  применение и проверенная перезагрузка PBR;
- обратимая пауза, которая сохраняет проверенную конфигурацию, и полное
  отключение;
- смена общего секрета, при которой выходная сторона держит установленное
  соединение;
- работает самостоятельно, без пакета IKEv2 Manager.
...
[Issues](https://github.com/Nikitid/luci-app-ikev2-site-link/issues/new/choose), выберите
подходящую форму. Об уязвимости сообщайте приватно через
[security advisory](https://github.com/Nikitid/luci-app-ikev2-site-link/security/advisories/new).
