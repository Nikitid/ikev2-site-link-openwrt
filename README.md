...
The monitor treats an installed SA as only one health signal. Source health also
requires the configured XFRM interface and ID, the exact live and fail-closed
routes, the PBR rule and a recent HTTPS probe through the current SA. Exit health
requires its permanent `/32` return route and evidence of traffic in both CHILD_SA
directions. Missing routes are reconciled on every monitor pass.
...
Applied interface names, XFRM IDs and the tunnel pool are recorded without
credentials. Disable the link before changing any of those resource identities
or switching roles; disable removes both role profiles and both owned XFRM
devices, so cleanup remains complete even if the form's role was changed first.
